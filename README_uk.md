...
4) Надсилаємо фрагмент обраному LLM-провайдеру із суворими підказками (prompts).
...
2) Запустіть первинне налаштування (ідемпотентно):

   ```bash
   ./init_once.sh
   ```

   Скрипт створить `.env` з `.env_default`, `config.yaml` з `config.yaml.example` і підготує `.venv` з залежностями.
3) Додайте ключі в `.env`:

   ```env
   OPENAI_API_KEY=ваш_ключ
   ```
4) Локальні перевизначення вносьте у `config.yaml` (НЕ редагуйте `config.default.yaml`).
...
- Оберіть провайдера в `config.yaml` (локальні перевизначення) у полі `llm.provider` (дефолт у `config.default.yaml`).
- Можна перевизначити через CLI: `--llm-provider openai|gemini|kie|evolink|groq|deepseek|dummy`.
...
- Конфіг: встановіть `logging.level` у `config.yaml` (локальні перевизначення) на `info`, `debug` або `trace`.
- CLI-перевизначення:
...
- `--context-file FILE` передає додаткові контекстні файли у `lecture_cleanup.sh` (можна вказувати кілька разів).
- `--debug` показує лог `yt-dlp` і прокидує `--debug` у `lecture_cleanup.sh`.
- Потрібно, щоб у відео були автогенеровані субтитри.

### YouTube-аудіо через Groq або Speechcore

Публічні `.sh`-скрипти використовують одну назву мовної опції:
`--lang CODE|auto`. Для автоматичної YouTube-обробки та playlist-варіантів
значення за замовчуванням — `auto`, тому його не потрібно вказувати явно.

```bash
# Groq сам визначить мову аудіо; субтитри YouTube не потрібні
...
# Явна мовна підказка однаково працює в single/list режимах
./auto_process_youtube_via_groq.sh --lang uk "https://youtu.be/ID"

# Speechcore single/list мають той самий публічний дефолт auto
...
У Groq режим `auto` передається безпосередньо провайдеру. У поточній
Speechcore-обгортці `auto` бере код мови з метаданих оригінальних субтитрів
YouTube, бо цей код потрібен для імені результату та cleanup. Якщо таких
метаданих немає, передайте `--lang CODE`. Повний актуальний список параметрів
кожного скрипту доступний через `--help`.
...
- `--lang`: дволітерний код ISO-639-1, наприклад `ru`, `uk`, `en`, `de`, `pl`, `es`.
...
## Конфігурація (`config.default.yaml` + `config.yaml`)

**Не редагуйте** `config.default.yaml` — це базові налаштування в репозиторії.  
Створіть `config.yaml` з `config.yaml.example` для локальних перевизначень (файл ігнорується Git).

Порядок завантаження: `config.default.yaml` → `config.yaml` (глибоке злиття; словники рекурсивно, списки замінюються повністю).

Секрети зберігайте в `.env` (копія з `.env_default`).
...
LLM
- `llm.provider`: `openai`, `gemini`, `kie`, `evolink`, `groq`, `deepseek`, `dummy` або власний адаптер.
